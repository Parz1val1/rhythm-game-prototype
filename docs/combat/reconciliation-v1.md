...
- `CombatV1` provides an isolated cadence and encounter-state seam that reuses
  ...
  publishes stable identity, action, authored offset, configurable handoff and
  approach lead, their combined scoreable due beat, and current musical position;
  authored events can expand into one to four same-beat targets. The phrase-event
  ...
  using the same schedule for movement and matching lane feedback. It now repeats
  the Settle-free core cadence
