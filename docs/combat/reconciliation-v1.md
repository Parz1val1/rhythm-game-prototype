...
| V1 target | Current evidence | Classification | Migration disposition |
|---|---|---|---|
| Skills combine musical effects with character-specific interactions | `CombatV1Skill` Resources own purpose copy, contribution metadata, bar duration, timed action events, and ordered effect adapters. Bright Motif is a two-bar melodic run that contributes Groove; Steadying Harmony is a three-bar chord pattern that restores Composure after correct execution and contributes no direct Groove. The #16 human playtest confirmed their physical/tactical contrast and clear effects. Both currently use Luthier's four-lane language; loadouts and other characters are absent. | **Partial** | Reuse the validated authored schedule/effect seam for party breadth rather than adding Skill-specific orchestrator branches. |
