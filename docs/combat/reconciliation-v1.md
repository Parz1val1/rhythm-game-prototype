...
| Tactical Vamps continue indefinitely with no time pressure | `CombatV1` leaves Tactical Vamp unchanged for any number of BeatClock cycles while its backing harness audio, clock, and timing/input subscriptions continue. One provisional `CONTINUE_ROUND` intent queues a complete input-free four-beat count-in with no opponent phrase events, publishes its BeatClock-derived progress, and immediately replaces the HUD's Tactical Vamp copy with a committed listening transition. Exactly one Enemy Phrase begins on the following whole beat and is visible before its first preview; repeated rounds, resolution, and teardown clear the transition presentation. No skill or action is auto-selected. | **Aligned seam** | Preserve indefinite waiting, explicit committed-transition feedback, and the full-bar, beat-aligned re-entry when the provisional command is replaced by selected skills and party rules. |
...
- `CombatV1` provides an isolated cadence and encounter-state seam that reuses
  ...
  `CombatV1` now repeats the Settle-free core cadence
