...
- `CombatV1` provides an isolated cadence and encounter-state seam that reuses
  ...
  target/lane feedback. `CombatV1ResponsePerformanceFeedback` consumes the same
  grade event for lane-specific musical one-shots and owns no timing source.
